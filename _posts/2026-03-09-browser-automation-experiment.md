...
summary: 当 AI 的智力已经能看穿网页底层逻辑时，我们所谓的“高级封装”到底是帮手还是累赘？
date: 2026-03-09
tags:
  - OpenClaw
  - 浏览器自动化
...
1. 1\. **Agent A** ：全副武装，用的是我们精心封装的 技能。它的原理是 **将复杂的 HTML 树抽象为精简的语义节点 (@ref)** ，让模型通过高级指令（如“帮我截个图”、“查下按钮在哪”）进行操作。
2. 2\. **Agent B** ：轻装上阵，直接调用 Native `browser` tool，通过 **托管浏览器 (Managed Browser)** 的 CDP 协议肉搏。
...
我发现 Agent A 迷路了。即便它加载了强大的 技能，但由于技能内部封装了太多的逻辑判定，在面对频繁的标签页切换时，这种“保姆式”服务反而成了模型的认知负担，导致它陷入了死循环。
...
![图片](https://liuhedev.github.io/images/2026-03-09/img-002-640.jpg)

| 指标 | Agent A (agent-browser) | Agent B (原子模式) |
| --- | --- | --- |
| **最终结果** | ❌ 失败 (迷路中) | ✅ **1分17秒 绝杀成功** |
...
![](https://liuhedev.github.io/images/2026-03-09/img-003-0.png)

刘贺同学

向上滑动看下一个

![](https://liuhedev.github.io/images/2026-03-09/img-004-300.png)
